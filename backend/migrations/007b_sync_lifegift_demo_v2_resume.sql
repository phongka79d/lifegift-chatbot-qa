-- ============================================================
-- 007b resume: continue v2 sync after products.category_id FK block
-- Safe to run only after 007 partially applied (new tables exist).
-- ============================================================
...
-- Abort if fully applied
SET @done = (SELECT COUNT(*) FROM _schema_migrations WHERE migration_name = '007_sync_lifegift_demo_v2');
-- Continue even if 0; final insert marks complete.
...
-- Drop FKs that use ON DELETE SET NULL before NOT NULL alters
...
-- Recreate FKs to match v2 (no ON DELETE SET NULL on category)
...
WHERE quantity = 0 OR quantity IS NULL OR quantity < reserved_quantity;
...
SET
  ...
  o.subtotal = CASE WHEN COALESCE(o.subtotal,0) = 0 THEN o.total_amount ELSE o.subtotal END;
...
-- ============================================================
-- 8. COUPONS rebuild (was empty / legacy shape)
-- ============================================================
...
SET @sql = IF(@fk IS NOT NULL, 'ALTER TABLE reviews DROP FOREIGN KEY fk_reviews_user', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@fk IS NOT NULL, 'ALTER TABLE reviews DROP FOREIGN KEY fk_reviews_product', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
-- Drop SET NULL FK before making category_id NOT NULL
SET @fk = (SELECT CONSTRAINT_NAME FROM information_schema.TABLE_CONSTRAINTS
  WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='blog_posts' AND CONSTRAINT_NAME='fk_blog_posts_category');
SET @sql = IF(@fk IS NOT NULL, 'ALTER TABLE blog_posts DROP FOREIGN KEY fk_blog_posts_category', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(COUNT(*)=0,
  'ALTER TABLE blog_posts ADD CONSTRAINT fk_blog_posts_category FOREIGN KEY (category_id) REFERENCES blog_categories(id)',
  'SELECT 1')
  FROM information_schema.TABLE_CONSTRAINTS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='blog_posts' AND CONSTRAINT_NAME='fk_blog_posts_category');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
